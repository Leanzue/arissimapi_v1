...
<div class="container">
    ...
    <table class="table mt-3">
        ...
        @foreach($treatmentresults as $treatmentresult)
            <tr>
                <td>{{ $treatmentresult->date_tentative }}</td>
                <td>{{ $treatmentresult->details }}</td>
                <td>{{ $treatmentresult->resultat }}</td>
                <td>{{ $treatmentresult->comment }}</td>
                <td class="actions-container">
                    <a href="{{ route('treatmentresults.create') }}" class="btn btn-primary">New</a>
                    <a href="{{ route('treatmentresults.show', $treatmentresult->id) }}" class="btn btn-warning">Show</a>
                    <a href="{{ route('treatmentresults.edit', $treatmentresult->id) }}" class="btn btn-warning">Edit</a>
                    <form action="{{ route('treatmentresults.destroy', $treatmentresult->id) }}" method="POST" style="display: inline-block;" class="delete-form">
                        @csrf
                        @method('DELETE')
